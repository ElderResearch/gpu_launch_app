...
JUPYTER_IMAGES = [k for (k, v) in ERI_IMAGES.items() if 8888 in v.get("ports", {})]
...
def active_eri_images(client=None, ignore_other_images=False):
    ...
    for c in client.containers.list():
        ...
        try:
            ...
            d["uptime"] = "{0:.0f} Days {1:02.0f}:{2:02.0f}:{3:02.0f}".format(
                *_calculate_uptime(t0=created)
            )
        except Exception as e:
            d["uptime"] = str(e)
...
def _find_open_port(start=8890, stop=9000):
    used_ports = {connection.laddr.port for connection in psutil.net_connections()}

    for port in range(start, stop + 1):
        if port not in used_ports:
            return port, None

    return (False, "unable to allocate open port between {} and {}".format(start, stop))
...
def launch(
    ...
    **kwargs
    ...
    if num_gpus > 0:
        ...
        _update_environment(imagedict, "NVIDIA_VISIBLE_DEVICES", ",".join(gpu_ids))
    else:
        _update_environment(imagedict, "NVIDIA_VISIBLE_DEVICES", "none")
...
def parse_args():
    ...
    username = (
        "username (must be a created user on the gpu box). "
        "Will be requested if not passed"
    )
    parser.add_argument("-u", "--username", help=username)

    password = (
        "password on gpu box. DO NOT PASS if running from command line. "
    ...
    parser.add_argument(
        "-t", "--imagetype", help=imagetype, choices=ERI_IMAGES.keys(), default="Python"
    )

    imagetag = 'tag of docker image to launch (default = "latest")'
    parser.add_argument("--imagetag", help=imagetag, default="latest")
